...
async function populateSelectFromRef(selectId, tableName) {
  ...
  refOptionsCache[selectId] = values;
...
function fillJobFormFromBooking(booking) {
  ...
  setValue("jobBookingDate", booking.date);
  setValue("jobCustomerName", booking.customerName);
  setValue("jobVehiclePlate", booking.vehiclePlate);
...
function onJobBookingSelected(event) {
  ...
  setMessage("jobMessage", `Loaded booking ${selected.bookingId} details.`, "success");
}
...
async function submitJob(event) {
  ...
  try {
    ...
    const selectedCondition = document.getElementById("jobOnSiteCondition").value;
    const selectedJobStatus = document.getElementById("jobStatus").value;

    const validCondition = (refOptionsCache.jobOnSiteCondition || []).includes(selectedCondition)
      ? selectedCondition
      : null;
    const validJobStatus = (refOptionsCache.jobStatus || []).includes(selectedJobStatus)
      ? selectedJobStatus
      : null;
    ...
    const payload = {
      ...
      on_site_condition: validCondition,
    ...
    event.target.reset();
    fillBookingIdFields();
    const linkedId = localStorage.getItem("booking_id") || bookingId;
    const linked = bookingCache.find((item) => item.bookingId === linkedId);
    if (linked) fillJobFormFromBooking(linked);
  } catch (error) {
    if (/job_sheet_on_site_condition_fkey/i.test(error.message || "")) {
      setMessage("jobMessage", "Invalid on-site condition value. Please select a value from ref_condition.", "error");
    } else if (/job_sheet_job_status_fkey/i.test(error.message || "")) {
      setMessage("jobMessage", "Invalid job status value. Please select a value from ref_job_status.", "error");
    } else {
